...
const Profile = () => {
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      if (selectedImageFile) {
        data.append("profileImage", selectedImageFile);
      }
      ...
      setTimeout(async () => {
        try {
          const refreshResponse = await axios.get(`http://localhost:5000/api/users/profile/${uniqueId}`);
  ...
  const getImageURL = () => {
    if (previewImageURL) {
      ...
      return `http://localhost:5000${previewImageURL.startsWith('/') ? '' : '/'}${previewImageURL}`;
    }
    return "";
  };
...
//     try {
//       console.log("Fetching profile with ID:", uniqueId); // Add this
//       const response = await axios.get(
//         `${process.env.REACT_APP_API_BASE}/api/users/profile/${uniqueId}`
//       );

//       console.log("Profile fetched:", response.data); // Add this
//       setFormData(response.data);
//       if (response.data.profileImage) {
//         setPreviewImageURL(response.data.profileImage);
//       }
//     } catch (error) {
//       console.error("Error fetching user data:", error); // Already added
//       if (error.response?.status === 401) {
//         handleLogout();
//       }
//     }
...
//       // Use REACT_APP_API_BASE for consistency
...
//       // Force refresh the profile data to get the latest image URL
//       // This timeout is a workaround if the server doesn't return the immediate accessible URL in the PUT response.
//       setTimeout(async () => {
//         try {
//           // Use REACT_APP_API_BASE for consistency
//           const refreshResponse = await axios.get(`${process.env.REACT_APP_API_BASE}/api/users/profile/${uniqueId}`);
//           setFormData(refreshResponse.data);
//           if (refreshResponse.data.profileImage) {
//             setPreviewImageURL(refreshResponse.data.profileImage);
//             // Update localStorage with the latest profile image
...
//         return previewImageURL;
//       }
//       // If it's a relative path, prepend server URL from env
