...
//       setTimeout(async () => {
//         try {
// const refreshResponse = await axios.get(`${process.env.REACT_APP_API_BASE}/api/users/profile/${uniqueId}`);          setFormData(refreshResponse.data);
...
const Profile = () => {
  ...
  // Corrected useEffect to fetch user data
  useEffect(() => {
    const fetchUserData = async () => {
      console.log("REACT_APP_API_BASE:", process.env.REACT_APP_API_BASE);
      try {
    ...
    // Only call the function if uniqueId is a valid value
    if (uniqueId) {
        fetchUserData();
    }
  }, [uniqueId, navigate]);
